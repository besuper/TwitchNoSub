...
// token. The player reads that list, renders the subscribe gate and never calls
// usher, so the worker patch below never gets a request to intercept. Clearing
// the list lets playback carry on to usher, where the patch takes over.
...
// ever asks for a token, so the restriction has to be cleared there as well. The
// field turns up under several operations, hence the blind walk.
//
